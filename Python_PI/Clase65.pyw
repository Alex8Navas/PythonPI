# Clase 65. Curso Píldoras Informáticas.

# Control de Flujo. Práctica Guiada 7. 

# Borrar Goblin y Consultas parametrizadas. 
...
def createGoblin(): # Crear un registro. 
	...
	# Con consulta paramétrica. Esto evita el uso fraudulento de bases de datos (que se pueda eliminar la base de datos).  
	dataGoblinCreation = SName.get(), SRace.get(), SVillage.get(), SPassword.get(), textDescription.get("1.0", END)

	puntero.execute("INSERT INTO GoblinsData VALUES(NULL, ?, ?, ?, ?, ?)", (dataGoblinCreation)) 
...
def updateGoblin(): # Actualizar un registro. 
	...
	# Con consulta parmétrica:
	dataGoblinUpdate = SName.get(), SRace.get(), SVillage.get(), SPassword.get(), textDescription.get("1.0", END)
	puntero.execute("UPDATE GoblinsData SET NAME= ?, RACE= ?, VILLAGE = ?, PASSWORD = ?, DESCRIPTION = ?" + 
		"WHERE ID=" + SID.get(), (dataGoblinUpdate))
...
def deleteGoblin():
	...
	puntero.execute("DELETE FROM GoblinsData WHERE ID=" + SID.get())

	conexion.commit()

	messagebox.showinfo("Goblin´s Base", "Lamentamos comunicar que un goblin ha fallecido")

def license():

	messagebox.showinfo("Goblin´s License", "El Señor de los Goblins le concede el sello de licencia CC")

def aboutUs():

	messagebox.showinfo("Goblin´s Message", "Aplicación desarrollada en el curso de Python de Píldoras Informáticas")
...
CRUDMenu.add_command(label = "Delete", command = deleteGoblin)
...
buttonDelete = Button(FrameButt, text = "Delete", command = deleteGoblin)
